  ...
  var component  = {
      ...
      html: {
        main: {
          inner: [
            ...
            { tag: 'hr' },
            { tag: 'table', inner: [
              { tag: 'tr', inner: [ { tag: 'th', inner: 'Nr', title: 'Nr des Autos' }, { tag: 'th', inner: 'Von', title: 'Startzeit des Parkens' }, { tag: 'th', inner: 'Bis', title: 'Endzeit des Parkens' }, { tag: 'th', inner: 'Dauer', title: 'Wie lange war das Auto im Parkhaus?' }, { tag: 'th', inner: 'Ticket', title: 'Ticket Hash' }, { tag: 'th', inner: 'Kennzeichen', title: 'License' }, { tag: 'th', inner: 'Preis', title: 'Parkgebühren' } ] }
            ] },
            { tag: 'div', class: 'errors', style: 'display: none;' }
          ]
        },
        row: { tag: 'tr', inner: [ { tag: 'td', inner: '%nr%' }, { tag: 'td', inner: '%von%' }, { tag: 'td', inner: '%bis%' }, { tag: 'td', inner: '%dauer%' }, { tag: 'td', inner: '%ticket%' }, { tag: 'td', inner: '%license%' }, { tag: 'td', class: 'price', inner: '%preis%' } ] },
    ...
    Instance: function () {
      ...
      this.start = async () => {
        ...
        class Garage {  // single source of truth for max and all counters
          ...
          async removeCar( car ){
            if ( car ){
              ...
              if ( ! self.json_format || self.json_format === "false" ){
                await csv_post_event( 'leave', ...car.toArray() );
              } else {
                await csv_post_event( 'leave', car.toJSON() );
              }
        ...
        function stop( e ){
          clearInterval( interval );  // ToDo is interval === false ?
