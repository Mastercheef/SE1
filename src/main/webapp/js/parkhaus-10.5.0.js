      ...
      this.start = async () => {
        ...
        class Car {
          ...
          leave() {
            ...
            return this._duration;  // duration is simulation time
          }
          gone(){
          ...
          beginTime(){  // simulation time
            const startTime = BigInt( self.time_shift || 0 );
            const result = BigInt( self.simulation_speed || 1 ) * BigInt( this.timer ) - startTime;
        ...
        if ( config_string && config_string.length > 1 ){
          ...
          const config = { max: parseInt(max), open_from: parseInt(open_from), open_to: parseInt(open_to), delay: parseInt(delay), simulation_speed: parseInt(simulation_speed) };
